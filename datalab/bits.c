...
int bitCount(int x) {
    ...
    int ans=(x&mask1)+((x>>1)&mask1);//2 bits
    ans=(ans&mask2)+((ans>>2)&mask2);//4 bits
    ans=(ans&mask3)+((ans>>4)&mask3);//8 bits
    ans=(ans&mask4)+((ans>>8)&mask4);//16 bits
    ans=(ans&mask5)+((ans>>16)&mask5);//32 bits
...
int fitsBits(int x, int n) {
    int sign=x>>31;
    int shift=n+(~0);
    return (!sign&!(x>>shift))+(sign&!((~x)>>shift));
}
...
int ilog2(int x) {
    /*
    log2(x)-[0,31] interpreted as 5-bits.
    log2(x)=16*a+8*b+4*c+2*d+e;
    a,b,c,d,e-[0,1]
...
unsigned float_neg(unsigned uf) {
    //if uf is equal to nan,return nan
...
unsigned float_twice(unsigned uf) {
    if((uf&0x7f800000)==0){
        //denormalized
        unsigned result=(uf&0x80000000)|((uf&0x007fffff)<<1);
        return result;
    }

    else if((uf&0x7f800000)!=0x7f800000){
        //normalized
        unsigned result=uf+(1<<23);
